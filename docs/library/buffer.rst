buffer
======

.. include:: <isonum.txt>

:mini:`fun buffer(Length: integer)` |rarr| :mini:`buffer`
   *Defined at line 1693 in src/ml_types.c*

:mini:`buffer`
   *Defined at line 1709 in src/ml_types.c*

:mini:`meth +(Buffer: buffer, Offset: integer)` |rarr| :mini:`buffer`
   *Defined at line 1714 in src/ml_types.c*

:mini:`meth -(Buffer₁: buffer, Buffer₂: buffer)` |rarr| :mini:`integer`
   *Defined at line 1729 in src/ml_types.c*

:mini:`meth string(Arg₁: buffer)`
   *Defined at line 1739 in src/ml_types.c*


...
static void ml_uv_fs_open_cb(uv_fs_t *Request) {
	ml_state_t *Frame = (ml_state_t *)Request->data;
	ml_value_t *Result;
	if (Request->result >= 0) {
...
static ml_spawn_t ml_uv_fs_open(ml_state_t *Frame, void *Data, int Count, ml_value_t **Args) {
...
static ml_spawn_t ml_uv_fs_close(ml_state_t *Frame, void *Data, int Count, ml_value_t **Args) {
...
static void ml_uv_fs_read_cb(ml_uv_fs_buf_t *Request) {
	ml_state_t *Frame = (ml_state_t *)Request->Base.data;
	ml_value_t *Result;
...
static ml_spawn_t ml_uv_fs_read(ml_state_t *Frame, void *Data, int Count, ml_value_t **Args) {
	ml_uv_file_t *File = (ml_uv_file_t *)Args[0];
	size_t Length = ml_integer_value(Args[1]);
	ml_uv_fs_buf_t *Request = xnew(ml_uv_fs_buf_t, 1, uv_buf_t);
	Request->Base.data = Frame;
...
static ml_spawn_t ml_uv_fs_write(ml_state_t *Frame, void *Data, int Count, ml_value_t **Args) {
	ml_uv_file_t *File = (ml_uv_file_t *)Args[0];
	ml_uv_fs_buf_t *Request = xnew(ml_uv_fs_buf_t, 1, uv_buf_t);
	Request->Base.data = Frame;

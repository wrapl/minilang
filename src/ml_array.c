...
static ml_array_t *ml_array_sub_alloc(ml_array_t *Source, int Degree) {
	if (ml_is_subtype(Source->Base.Type, MLArrayMutableT)) {
		return ml_array_alloc(Source->Format, Degree);
	} else {
		return ml_array_const_alloc(Source->Format, Degree);
	}
}

void *array_alloc(ml_array_format_t Format, size_t Size) {
	if (Format == ML_ARRAY_FORMAT_ANY) return anew(ml_value_t *, Size / sizeof(ml_value_t *));
	return snew(Size);
}
...
//$= array::wrap(array::uint16, B, [2, 2, 2], [8, 4, 2])
	...
	if (Format == ML_ARRAY_FORMAT_ANY) return ml_error("TypeError", "Invalid type for operation");
	...
	if (ml_is(Args[1], MLBufferT)) {
		Array = ml_array_alloc(Format, Degree);
	} else {
		Array = ml_array_const_alloc(Format, Degree);
	}
	size_t Total = 0;
...
static ml_array_t *ml_array_transpose(ml_array_t *Source) {
	...
	ml_array_t *Target = ml_array_sub_alloc(Source, Degree);
	for (int I = 0; I < Degree; ++I) {
...
//    This method always makes a copy of the data so that changes to the returned array do not affect the original.
	...
	ml_array_t *Target = ml_array_sub_alloc(Source, TargetDegree);
	Target->Base.Value = ml_array_flatten(Source);
...
// Returns an array sharing the underlying data with :mini:`Array` replacing the dimension at :mini:`Index` with new dimensions with sizes :mini:`Sizes`. The total count :mini:`Sizes/1 * Sizes/2 * ... * Sizes/n` must equal the original size.
	...
	ml_array_t *Target = ml_array_sub_alloc(Source, Degree + ml_list_length(Args[2]) - 1);
...
// Returns an array sharing the underlying data with :mini:`Array` replacing the dimensions at :mini:`Start .. (Start + Count)` with a single dimension with the same overall size.
	...
	ml_array_t *Target = ml_array_sub_alloc(Source, Degree + 1 - Join);
	ml_array_dimension_t *SourceDimension = Source->Dimensions + Degree;
	ml_array_dimension_t *TargetDimension = Target->Dimensions + Target->Degree;
...
static ml_value_t *ML_TYPED_FN(ml_array_index_get, MLArrayUInt8T, ml_array_t *Array, ml_array_indexer_t *Indexer) {
	int Degree = Array->Degree;
	if (Indexer->Source + Degree > Indexer->Limit) return ml_error("IntervalError", "Too many indices");
	for (int I = 0; I < Degree; ++I) {
		if (Array->Dimensions[I].Size != Indexer->Source[I].Size) {
...
static ml_value_t *ML_TYPED_FN(ml_array_index_get, MLArrayUInt32T, ml_array_t *Array, ml_array_indexer_t *Indexer) {
...
ml_value_t *ml_array_index(ml_array_t *Source, int Count, ml_value_t **Indices) {
	...
	if (ml_is_subtype(Source->Base.Type, MLArrayMutableT)) {
		...
		ml_array_t *Val = ml_array_const_alloc(Source->Format, Degree);
...
#define ML_ARRAY_ITER_FN(SUFFIX, CTYPE, TO_VAL, FROM_VAL) \
\
static ml_value_t *ml_array_iter_ref_deref_ ## CTYPE(ml_array_iter_ref_t *Ref) { \
...
\
static void ml_array_iter_ref_assign_ ## CTYPE(ml_state_t *Caller, ml_array_iter_ref_t *Ref, ml_value_t *Value) { \
...
ML_ARRAY_ITER_FN(UInt8T, uint8_t, ml_integer, ml_integer_value);
ML_ARRAY_ITER_FN(Int8T, int8_t, ml_integer, ml_integer_value);
ML_ARRAY_ITER_FN(UInt16T, uint16_t, ml_integer, ml_integer_value);
ML_ARRAY_ITER_FN(Int16T, int16_t, ml_integer, ml_integer_value);
ML_ARRAY_ITER_FN(UInt32T, uint32_t, ml_integer, ml_integer_value);
ML_ARRAY_ITER_FN(Int32T, int32_t, ml_integer, ml_integer_value);
ML_ARRAY_ITER_FN(UInt64T, uint64_t, ml_integer, ml_integer_value);
ML_ARRAY_ITER_FN(Int64T, int64_t, ml_integer, ml_integer_value);
ML_ARRAY_ITER_FN(Float32T, float, ml_real, ml_real_value);
ML_ARRAY_ITER_FN(Float64T, double, ml_real, ml_real_value);

#ifdef ML_COMPLEX
ML_ARRAY_ITER_FN(Complex32T, complex_float, ml_complex, ml_complex_value);
ML_ARRAY_ITER_FN(Complex64T, complex_double, ml_complex, ml_complex_value);
#endif

ML_ARRAY_ITER_FN(AnyT, any, , );
...
static void ML_TYPED_FN(ml_iter_value, MLArrayIteratorT, ml_state_t *Caller, ml_array_iterator_t *Iterator) {
	ML_RETURN(Iterator->deref(Iterator->Address));
}

ML_TYPE(MLArrayMutableIteratorT, (MLArrayIteratorT), "array::mutable::iterator");

static void ML_TYPED_FN(ml_iter_value, MLArrayMutableIteratorT, ml_state_t *Caller, ml_array_iterator_t *Iterator) {
	ml_array_iter_ref_t *Ref = new(ml_array_iter_ref_t);
	Ref->Type = Iterator->RefType;
	Ref->Address = Iterator->Address;
	ML_RETURN(Ref);
}
...
static void ML_TYPED_FN(ml_iterate, MLArrayMutableT, ml_state_t *Caller, ml_array_t *Array) {
	...
	switch (Array->Format) {
	case ML_ARRAY_FORMAT_U8:
		Iterator->RefType = MLRefUInt8T;
		break;
	case ML_ARRAY_FORMAT_I8:
		Iterator->RefType = MLRefInt8T;
		break;
	case ML_ARRAY_FORMAT_U16:
		Iterator->RefType = MLRefUInt16T;
		break;
	case ML_ARRAY_FORMAT_I16:
		Iterator->RefType = MLRefInt16T;
		break;
	case ML_ARRAY_FORMAT_U32:
		Iterator->RefType = MLRefUInt32T;
		break;
	case ML_ARRAY_FORMAT_I32:
		Iterator->RefType = MLRefInt32T;
		break;
	case ML_ARRAY_FORMAT_U64:
		Iterator->RefType = MLRefUInt64T;
		break;
	case ML_ARRAY_FORMAT_I64:
		Iterator->RefType = MLRefInt64T;
		break;
	case ML_ARRAY_FORMAT_F32:
		Iterator->RefType = MLRefFloat32T;
		break;
	case ML_ARRAY_FORMAT_F64:
		Iterator->RefType = MLRefFloat64T;
		break;
#ifdef ML_COMPLEX
	case ML_ARRAY_FORMAT_C32:
		Iterator->RefType = MLRefComplex32T;
		break;
	case ML_ARRAY_FORMAT_C64:
		Iterator->RefType = MLRefComplex64T;
		break;
#endif
	case ML_ARRAY_FORMAT_ANY:
		Iterator->RefType = MLRefAnyT;
		break;
	default:
		ML_ERROR("TypeError", "Invalid array type for iteration");
	}
...
#define ARRAY_NUMERIC_DECL(PARENT, PREFIX, SUFFIX, CTYPE, APPEND, PRINTF, FROM_VAL, TO_VAL, FROM_NUM, TO_NUM, FORMAT, HASH) \
\
ARRAY_DECL(PARENT, PREFIX, SUFFIX, CTYPE, APPEND, PRINTF, FROM_VAL, TO_VAL, FROM_NUM, TO_NUM, FORMAT, HASH) \
\
...
ARRAY_NUMERIC_DECL(IntegerT, uint8, UInt8T, uint8_t, ml_stringbuffer_printf, "%u", ml_integer_value, ml_integer, , NOP_VAL, ML_ARRAY_FORMAT_U8, (long));
ARRAY_NUMERIC_DECL(IntegerT, int8, Int8T, int8_t, ml_stringbuffer_printf, "%d", ml_integer_value, ml_integer, , NOP_VAL, ML_ARRAY_FORMAT_I8, (long));
ARRAY_NUMERIC_DECL(IntegerT, uint16, UInt16T, uint16_t, ml_stringbuffer_printf, "%u", ml_integer_value, ml_integer, , NOP_VAL, ML_ARRAY_FORMAT_U16, (long));
ARRAY_NUMERIC_DECL(IntegerT, int16, Int16T, int16_t, ml_stringbuffer_printf, "%d", ml_integer_value, ml_integer, , NOP_VAL, ML_ARRAY_FORMAT_I16, (long));
ARRAY_NUMERIC_DECL(IntegerT, uint32, UInt32T, uint32_t, ml_stringbuffer_printf, "%u", ml_integer_value, ml_integer, , NOP_VAL, ML_ARRAY_FORMAT_U32, (long));
ARRAY_NUMERIC_DECL(IntegerT, int32, Int32T, int32_t, ml_stringbuffer_printf, "%d", ml_integer_value, ml_integer, , NOP_VAL, ML_ARRAY_FORMAT_I32, (long));
ARRAY_NUMERIC_DECL(IntegerT, uint64, UInt64T, uint64_t, ml_stringbuffer_printf, "%" PRIu64, ml_integer_value, ml_integer, , NOP_VAL, ML_ARRAY_FORMAT_U64, (long));
ARRAY_NUMERIC_DECL(IntegerT, int64, Int64T, int64_t, ml_stringbuffer_printf, "%" PRId64, ml_integer_value, ml_integer, , NOP_VAL, ML_ARRAY_FORMAT_I64, (long));
ARRAY_NUMERIC_DECL(RealT, float32, Float32T, float, ml_stringbuffer_printf, "%g", ml_real_value, ml_real, , NOP_VAL, ML_ARRAY_FORMAT_F32, (long));
ARRAY_NUMERIC_DECL(RealT, float64, Float64T, double, ml_stringbuffer_printf, "%g", ml_real_value, ml_real, , NOP_VAL, ML_ARRAY_FORMAT_F64, (long));

#ifdef ML_COMPLEX

ARRAY_NUMERIC_DECL(ComplexT, complex32, Complex32T, complex_float, COMPLEX_APPEND, "%g", ml_complex_value, ml_complex, , NOP_VAL, ML_ARRAY_FORMAT_C32, (long));
ARRAY_NUMERIC_DECL(ComplexT, complex64, Complex64T, complex_double, COMPLEX_APPEND, "%g", ml_complex_value, ml_complex, , NOP_VAL, ML_ARRAY_FORMAT_C64, (long));
...
size_t ml_array_data_size(ml_array_t *Source) {
	...
	for (int I = Source->Degree; --I >= 0;) Size *= Source->Dimensions[I].Size;
	return Size;
}
...
char *ml_array_flatten(ml_array_t *Source) {
	size_t Size = ml_array_data_size(Source);
	char *Data = array_alloc(Source->Format, Size);
	ml_array_copy_data(Source, Data);
	return Data;
}

extern void update_array(update_row_fn_t Update, ml_array_dimension_t *TargetDimension, char *TargetData, int SourceDegree, ml_array_dimension_t *SourceDimension, char *SourceData);
...
int ml_array_copy(ml_array_t *Target, ml_array_t *Source) {
	...
	if (Target->Format == Source->Format) {
		...
		Target->Base.Value = array_alloc(Target->Format, DataSize);
		update_row_fn_t Update = UpdateSetRowFns[Target->Format * MAX_FORMATS + Source->Format];
...
ML_METHOD("const", MLVisitorT, MLArrayT) {
	...
	ml_array_t *Source = (ml_array_t *)Args[1];
	if (Source->Degree == -1) return (ml_value_t *)Source;
	ml_array_t *Target = ml_array_const_alloc(Source->Format, Source->Degree);
	inthash_insert(Visitor->Cache, (uintptr_t)Source, Target);
	if (Source->Format == ML_ARRAY_FORMAT_ANY) {
		ml_array_copy(Target, Source);
		// TODO: Use Visitor to make a copy of each value in Target.
...
// Return a new array with the results of applying :mini:`Function` to each value of :mini:`Array`.
	...
	int Remaining = ml_array_copy(C, A) / MLArraySizes[C->Format];
...
extern void ml_array2_init(stringmap_t *Globals);

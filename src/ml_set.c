...
ML_ENUM2_WITH_ID(MLSetOrderT, "set::order",
...
ML_METHOD("subsets", MLSetT) {
	...
	ML_SET_FOREACH(Set, Iter) *Values++ = Iter->Key;
	return ml_subsets(Values - N, INT_MAX, N, ml_set, (void *)ml_set_insert);
}
...
ML_METHOD("subsets", MLSetT, MLIntegerT) {
	...
	ml_value_t **Values = anew(ml_value_t *, N);
	ML_SET_FOREACH(Set, Iter) *Values++ = Iter->Key;
	return ml_subsets(Values - N, M, N, ml_set, (void *)ml_set_insert);
}

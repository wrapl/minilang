...
void ml_return_expr_compile(mlc_function_t *Function, mlc_parent_expr_t *Expr, int Flags) {
	...
	} else if (Function->Must || Function->Try) {
		MLC_FRAME(mlc_parent_expr_frame_t, ml_return_expr_compile2);
		Frame->Expr = Expr;
		Frame->Flags = Flags;
		MLC_EMIT(Expr->StartLine, MLI_NIL, 0);
		return ml_return_expr_compile2(Function, NULL, Frame);
	} else {
		MLC_EMIT(Expr->StartLine, MLI_NIL, 0);
		MLC_EMIT(Expr->EndLine, MLI_RETURN, 0);
		MLC_RETURN(MLExprGoto);
...
static __attribute__((noinline)) void ml_block_expr_compile0(mlc_function_t *Function, mlc_block_expr_t *Expr, mlc_block_t *Frame) {
...
void ml_block_expr_compile(mlc_function_t *Function, mlc_block_expr_t *Expr, int Flags) {
	...
	ml_block_expr_compile0(Function, Expr, Frame);
	mlc_expr_t *Child = Expr->Child;
	if (Child) {
		Frame->Child = Child->Next;
		if ((Frame->Flags & MLCF_RETURN) && !Child->Next && !Function->Try) {
...
static void ml_call_expr_compile5(mlc_function_t *Function, ml_value_t *Value, ml_call_expr_frame_t *Frame) {
	...
	if (Frame->Value) {
		...
		if (ml_typeof(Frame->Value) == MLMethodT) {
			...
			if (Frame->CacheSlot[0] && Frame->CacheSlot[0]->Data) CallInst[3].Data = Frame->CacheSlot[0]->Data;
			Frame->CacheSlot[0] = &CallInst[3];
...
static __attribute__((noinline)) void ml_fun_expr_compile0(mlc_function_t *Function, mlc_fun_expr_t *Expr, mlc_function_t *SubFunction, ml_closure_info_t *Info, int *HasParamTypes) {
	int NumParams = 0;
	inthash_t DeclHashes[1] = {INTHASH_INIT};
...
void ml_fun_expr_compile(mlc_function_t *Function, mlc_fun_expr_t *Expr, int Flags) {
	...
	int HasParamTypes = 0;
	ml_fun_expr_compile0(Function, Expr, SubFunction, Info, &HasParamTypes);
	...
	Frame->Flags = Flags;
	return mlc_compile(SubFunction, Expr->Body, MLCF_RETURN);
}
...
static mlc_expr_t *ml_accept_term(ml_parser_t *Parser, ml_term_mode_t Mode);
...
static mlc_expr_t *ml_parse_factor(ml_parser_t *Parser, ml_term_mode_t Mode);
static mlc_expr_t *ml_parse_term(ml_parser_t *Parser, ml_term_mode_t Mode);
...
static mlc_expr_t *ml_accept_fun_expr(ml_parser_t *Parser, const char *Name, ml_token_t EndToken) {
	...
	if (!ml_parse2(Parser, EndToken)) {
		...
		do {
			...
			if (ml_parse2(Parser, MLT_LEFT_SQUARE)) {
				...
				if (ml_parse2(Parser, MLT_COLON)) Param->Type = ml_accept_term(Parser, TERM_NORMAL);
	...
	if (ml_parse2(Parser, MLT_COLON)) FunExpr->ReturnType = ml_accept_term(Parser, TERM_NORMAL);
...
static mlc_expr_t *ml_accept_meth_expr(ml_parser_t *Parser) {
	...
	mlc_expr_t *Method = ml_accept_term(Parser, TERM_METHOD_EXPR);
	if (!Method) {
...
static void ml_accept_named_arguments(ml_parser_t *Parser, ml_token_t EndToken, mlc_expr_t **ArgsSlot, ml_value_t *Names, mlc_expr_t *Value) {
	...
	ArgsSlot = &Arg->Next;
	int EOL = ml_parse(Parser, MLT_EOL);
	while (ml_parse2(Parser, MLT_COMMA) || EOL) {
...
with_name:
	...
	case MLT_LEFT_SQUARE: {
		...
		if (!ml_parse2(Parser, MLT_RIGHT_SQUARE)) {
			...
			do {
				ml_skip_eol(Parser);
				mlc_expr_t *Arg = ml_parse_expression(Parser, EXPR_DEFAULT);
				if (!Arg) break;
				EOL = ml_parse(Parser, MLT_EOL);
				ArgsSlot[0] = Arg;
				ArgsSlot = &Arg->Next;
	...
	case MLT_OPERATOR: {
		...
		if (Mode == TERM_METHOD_EXPR) {
			...
			mlc_expr_t *Child = ml_parse_term(Parser, TERM_NORMAL);
			if (Child) {
				ML_EXPR(CallExpr, parent_value, const_call);
				CallExpr->Value = Operator;
...
static mlc_expr_t *ml_parse_term_postfix(ml_parser_t *Parser, ml_term_mode_t Mode, mlc_expr_t *Expr) {
	for (;;) {
		switch (ml_current(Parser)) {
		case MLT_LEFT_PAREN: {
			if (Mode == TERM_METHOD_EXPR) return Expr;
			ml_next(Parser);
			ML_EXPR(CallExpr, parent, call);
			CallExpr->Child = Expr;
			ml_accept_arguments(Parser, MLT_RIGHT_PAREN, &Expr->Next);
		...
		case MLT_IS: {
			if (Mode != TERM_EXPR_DECL) return Expr;
			ml_next(Parser);
			ML_EXPR(CallExpr, parent, call);
			CallExpr->Child = Expr;
			ml_accept_arguments(Parser, MLT_END, &Expr->Next);
			Expr = ML_EXPR_END(CallExpr);
			break;
		}
		case MLT_LEFT_SQUARE: {
...
static mlc_expr_t *ml_parse_term(ml_parser_t *Parser, ml_term_mode_t Mode) {
	mlc_expr_t *Expr = ml_parse_factor(Parser, Mode);
	if (!Expr) return NULL;
	return ml_parse_term_postfix(Parser, Mode, Expr);
}

static mlc_expr_t *ml_accept_term(ml_parser_t *Parser, ml_term_mode_t Mode) {
	ml_skip_eol(Parser);
	mlc_expr_t *Expr = ml_parse_term(Parser, Mode);
	if (!Expr) {
...
static mlc_expr_t *ml_parse_expression(ml_parser_t *Parser, ml_expr_level_t Level) {
	mlc_expr_t *Expr = ml_parse_term(Parser, Level == EXPR_DECL ? TERM_EXPR_DECL : TERM_NORMAL);
	if (!Expr) return NULL;
		...
		if (ml_parse2(Parser, MLT_LEFT_PAREN)) {
			...
			if (Expr2) {
				if (Expr2->Next) {
					...
					Expr->Next = ml_parse_term_postfix(Parser, TERM_NORMAL, Expr2);
				}
			}
		} else {
			Expr->Next = ml_accept_term(Parser, TERM_NORMAL);
		}
		Expr = ML_EXPR_END(CallExpr);
		break;
	}
	...
	do {
		if (ml_parse2(Parser, MLT_LEFT_PAREN)) {
			...
			if (ml_parse(Parser, MLT_COLON)) {
				...
				TypeExpr->Child = ml_accept_term(Parser, TERM_NORMAL);
...
static void ml_accept_block_def(ml_parser_t *Parser, ml_accept_block_t *Accept) {
	do {
		if (ml_parse2(Parser, MLT_LEFT_PAREN)) {
			...
			mlc_expr_t *TypeExpr = ml_parse(Parser, MLT_COLON) ? ml_accept_term(Parser, TERM_NORMAL) : NULL;
...
static mlc_expr_t *ml_parse_block_expr(ml_parser_t *Parser, ml_accept_block_t *Accept) {
	mlc_expr_t *Expr = ml_parse_expression(Parser, EXPR_DECL);
	if (!Expr) return NULL;
	if (ml_parse(Parser, MLT_COLON)) {
		if (ml_parse2(Parser, MLT_VAR)) {
...
static void ml_command_evaluate_decl2(mlc_function_t *Function, ml_parser_t *Parser, ml_token_t Type) {
	if (ml_parse(Parser, MLT_LEFT_PAREN)) {
		...
		if (ml_parse(Parser, MLT_LEFT_PAREN)) {
			...
			if (ml_parse(Parser, MLT_COLON)) Frame->VarType = ml_accept_term(Parser, TERM_NORMAL);
			if (Type == MLT_VAR) {
				if (ml_parse(Parser, MLT_ASSIGN)) {
					mlc_expr_t *Expr = ml_accept_expression(Parser, EXPR_DEFAULT);
					return mlc_expr_call(Function, Expr);
...
static __attribute__ ((noinline)) void ml_command_evaluate_expr(mlc_function_t *Function, ml_parser_t *Parser) {
	...
	mlc_expr_t *Expr = ml_accept_expression(Parser, EXPR_DEFAULT);
	if (ml_parse(Parser, MLT_COLON)) {
		// TODO: Use the same parsing code as ml_parse_block_expr to allow nested expressions
		ml_accept(Parser, MLT_IDENT);
		const char *Ident = Parser->Ident;
		ML_EXPR(CallExpr, parent, call);
		CallExpr->Child = Expr;
		if (ml_parse(Parser, MLT_IS)) {
			ml_accept_arguments(Parser, MLT_END, &Expr->Next);
		} else {
			ml_accept(Parser, MLT_LEFT_PAREN);
			ml_accept_arguments(Parser, MLT_RIGHT_PAREN, &Expr->Next);
